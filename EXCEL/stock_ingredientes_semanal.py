...
def weekly_dicts_all_months(pizzas: list[Pizza]=None, orders: list[Order]=None) -> dict:

    d = {}
    
    if (not pizzas) or (not orders):
        pizzas, orders = extract()
    
    for i in range(len(MONTHS)):
        optimized_ingredients = transform(pizzas, orders, time_stamp=(i+1))
